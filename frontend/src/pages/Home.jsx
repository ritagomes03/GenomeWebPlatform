      ...
      <section className="relative min-h-screen md:min-h-[870px] flex flex-col items-center justify-center px-4 sm:px-6 lg:px-8">
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          <img
            ...
            className="absolute inset-0 w-full h-full object-cover dark:opacity-30 opacity-20"
          />
          ...
          <div className="max-w-screen-2xl mx-auto">
            <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-4">
              <div className="max-w-xl">
                <h3 className="font-[Space_Grotesk,system-ui,sans-serif] text-4xl font-bold dark:text-slate-100 text-slate-900 mb-4">
                  Species Showcase
                </h3>
                <p className="dark:text-slate-400 text-slate-500 text-lg">
                  Species with the highest number of genomic sequences currently recorded in the database.
                </p>
              </div>
              <button
                onClick={() => navigate('/species')}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {data?.top_species?.map((s) => (
                <SpeciesCard
                  ...
                  species={s}
                  onClick={() =>
                    navigate(`/species/${s.id}-${s.species.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`)
                  }
                />
              ))}
            </div>
